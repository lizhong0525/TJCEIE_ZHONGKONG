...
* 所有可现场标定的参数（相机内参/畸变、手眼矩阵、台面尺寸、6 按钮坐标、
...
def _to_buttons(items: Iterable[Any]) -> list[Button]:
    out: list[Button] = []
    for it in items or []:
        out.append(
            Button(
                name=_to_str(it.get("name") if isinstance(it, dict) else ""),
                color=_to_str(it.get("color") if isinstance(it, dict) else "red"),
                kind=_to_str(it.get("kind") if isinstance(it, dict) else "push"),
...
def from_dict(raw: dict[str, Any]) -> SiteConfig:
    ...
    panel = Panel(
        center=_to_vec3(panel_raw.get("center")),
        buttons=_to_buttons(panel_raw.get("buttons", [])),
    )
