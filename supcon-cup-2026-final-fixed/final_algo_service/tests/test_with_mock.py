...
def test_task2():
    ...
    任务二已切换为 task2_service 独立实现（真实物理槽ROI + 拒猜 +
    四放置点 + 姿态保持 + 放后验证 + 全成才成功），其行为测试见:
      python -m unittest tests.test_task2_controller -v
    旧 execute_cube_task（数字绑槽位 + 枚举补全）已删除。
    """
    print("\n" + "=" * 50)
    print("测试: /api/task2/execute → 已切换到 task2_service 实现")
    print("  行为测试: python -m unittest tests.test_task2_controller -v")
