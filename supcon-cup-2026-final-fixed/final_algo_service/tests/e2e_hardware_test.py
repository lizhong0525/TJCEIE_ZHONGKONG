...
from tasks import task1_switch, task3_shapes
...
print("任务2: 长方体有序转运")
print("=" * 60)
# 任务2 已切换为 task2_service 独立实现（真实物理槽ROI + 拒猜 + 四放置点
# + 姿态保持 + 全成才成功），不再走 tasks/task2_cubes.py。
# 行为测试：python -m unittest tests.test_task2_controller -v
# 真机/假服务器端到端需先填 config.json（占位配置会被拒绝），本脚本不再覆盖。
results["task2"] = {"ok": None, "message": "已切换到 task2_service，见 test_task2_controller"}
print("  跳过: 已切换到 task2_service 实现，行为测试见 tests/test_task2_controller.py")
