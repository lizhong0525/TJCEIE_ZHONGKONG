...
├── task2_service/            # 任务2: 有序转运（独立实现，见 任务二使用说明.md）
│   ├── controller.py         # 业务编排：识别→按序抓放→放后验证→全成才成功
│   ├── vision.py             # 物理槽ROI识别 + RGB-D精修（可选）
│   ├── camera.py             # Gemini335 采集（pyorbbecsdk2，D2C对齐）
│   ├── ocr.py                # EasyOCR/Tesseract/模板三路数字识别
│   └── config.json 见根目录   # 任务二现场配置（占位符未填完拒动真机）
├── config.json               # 任务二配置（__现场标定后填入__ 占位）
├── scripts/
