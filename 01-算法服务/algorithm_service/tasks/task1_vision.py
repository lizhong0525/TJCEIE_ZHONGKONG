...
若多个灯同时超阈值（现场反光等），取得分最高者并记 warning。
...
def detect_lit_lamp(
    ...
    """返回 ``(亮灯 name 或 None, ROI 未标定的 lamp name 列表)``。

    规则约定每次只亮一个灯；多个超阈值时取得分最高者。
    """
    ...
    for lamp in lamps:
        ...
        lit_by_color = color_ratio >= min_area_ratio
        lit_by_bright = mean_v >= bright_mean_min
        LOG.info(
            "lamp %s(%s): color_ratio=%.3f mean_v=%.1f -> %s",
            name, cname, color_ratio, mean_v,
            "LIT" if (lit_by_color or lit_by_bright) else "off",
        )
        if not (lit_by_color or lit_by_bright):
            continue
        # 归一化得分：两通道各自相对阈值的倍数，取大者
        score = max(
    ...
    return (best[0] if best else None), invalid
